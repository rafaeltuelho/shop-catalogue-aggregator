= Shop products catalogue api aggregator

The service demonstrates how to aggregate JSON data from 2 services using Apache Camel. 

* Execute an HTTP GET request on the `camel/catalogue` endpoint to retrieve a list of flights (arrivals and departures). Arrivals and Departures are aggregated from 2 separate services. See `CamelRouter.java` for how these 2 services are aggregated.
